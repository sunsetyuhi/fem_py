    ...
    for e in range(len(nod_pos_seg)):
        if(nod_pos_seg[e,0,0] <= x_min+0.1*leng_seg[e] and
           nod_pos_seg[e,1,0] <= x_min+0.1*leng_seg[e]):  #左側境界
            BC_type[e] = BC_left
            #BC_type[e,:] = BC_left[:]
        elif(x_max-0.1*leng_seg[e] <= nod_pos_seg[e,0,0] and
             x_max-0.1*leng_seg[e] <= nod_pos_seg[e,1,0]):  #右側境界
            BC_type[e] = BC_right
            #BC_type[e,:] = BC_right[:]
        elif(nod_pos_seg[e,0,1] <= y_min+0.1*leng_seg[e] and
             nod_pos_seg[e,1,1] <= y_min+0.1*leng_seg[e]):  #下側境界
            BC_type[e] = BC_bottom
            #BC_type[e,:] = BC_bottom[:]
        elif(y_max-0.1*leng_seg[e] <= nod_pos_seg[e,0,1] and
             y_max-0.1*leng_seg[e] <= nod_pos_seg[e,1,1]):  #上側境界
            BC_type[e] = BC_top
            #BC_type[e,:] = BC_top[:]
    print('BC_type =\n', BC_type)

    return BC_type, leng_seg


#境界条件を実装
def set_boundary_condition(mat_A_glo, vec_b_glo, BC_type, leng_seg):
...
#メッシュを表示
def visualize_mesh(show_text, out_type):
...
if __name__ == '__main__':
    ...
    #node_type = ['random', 10]  #節点の生成方法。lattice,random
    ...
    #境界要素の情報を設定
    BC_type, leng_seg = make_boundary_info(nod_pos_seg)

    #境界条件を実装
    mat_A_glo, vec_b_glo = set_boundary_condition(mat_A_glo, vec_b_glo, BC_type, leng_seg)
